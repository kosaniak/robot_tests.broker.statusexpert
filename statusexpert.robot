...
Підготувати клієнт для користувача
  ...
  Set Global Variable   ${STATUSEXPERT_LOT_ID}   ${EMPTY}
  Set Global Variable   ${STATUSEXPERT_MODIFICATION_DATE}   ${EMPTY}
  ...
  Run Keyword If  '${username}' != 'statusexpert_Viewer'  Login  ${username}
...
Створити тендер
  ...
  ${guarantee}=           Convert To String   ${guarantee}
   ...
   Set Global Variable   ${STATUSEXPERT_LOT_ID}   ${lotID}
   Click Element  id=btn_finished
   Wait Until Page Contains  Заявка на торги  30
   ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
   ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _publish_lot
   ...
   Wait Until Element Is Visible   xpath=//a[contains(@href,'${STATUSEXPERT_LOT_ID}')]   15
   Click Link   xpath=//a[contains(@href,'${STATUSEXPERT_LOT_ID}')]
...
Перейти в розділ додавання активів
  ...
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _edit_lot
  Клацнути по випадаючому списку   ${drop_id}
  Виконати дію   ${action_id}
...
Пошук тендера по ідентифікатору
    ...
    ${flag}=  Run Keyword And Return Status  Should Be Empty  ${STATUSEXPERT_LOT_ID}
...
Пошук тендера у разі наявності змін
  ...
  ${status}=   Run Keyword And Return Status   Should Not Be Equal   ${STATUSEXPERT_MODIFICATION_DATE}   ${last_mod_date}
  Run Keyword If   ${status}   statusexpert.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
  Set Global Variable   ${STATUSEXPERT_MODIFICATION_DATE}   ${last_mod_date}

Завантажити документ в тендер з типом
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}  ${documentType}
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _add_files
  Клацнути по випадаючому списку   ${drop_id}
  Виконати дію   ${action_id}
  Wait Until Element Is Visible   xpath=//input[@type='submit']   10
  ${inputID}=   convert_statusexpert_string_to_common_string   ${documentType}
...
Отримати кількість предметів в тендері
    ...
    statusexpert.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
...
Чи фінасова процедура
  ...
  ${procedureType}=   convert_statusexpert_string_to_common_string   ${procedureType}
...
Подати цінову пропозицію
  ...
  statusexpert.Пошук тендера по ідентифікатору   ${username}   ${auction_id}
...
Завантажити документ в ставку
  ...
  ...      ${ARGUMENTS[1]} ==  ${filepath}
  ...      ${ARGUMENTS[2]} ==  ${TENDER_UAID}
  statusexpert.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}   ${ARGUMENTS[2]}
...
Оновити сторінку з тендером
  ...
  ...      ${ARGUMENTS[1]} =  ${TENDER_UAID}
  Switch Browser   ${BROWSER_ALIAS}
  Go To  ${BROKERS['statusexpert'].syncpage}
  Go To  ${BROKERS['statusexpert'].homepage}
  statusexpert.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Відправити документи до цбд
  ...
  Go To  ${BROKERS['statusexpert'].syncdocs}
  Go to  ${location}
...
Отримати інформацію із тендера
  ...
  Run Keyword If    "${resp}" == "False"   statusexpert.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Отримати інформацію про status
  ...
  ${status}=   convert_statusexpert_string_to_common_string   ${return_value}
  [return]   ${status}
...
Отримати посилання на аукціон
  ...
  Run Keyword If    ${isAuctionView} == ${False}   statusexpert.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}  ${ARGUMENTS[1]}
...
Завантажити протокол аукціону
  ...
  statusexpert.Оновити сторінку з тендером  ${ARGUMENTS[0]}  ${ARGUMENTS[1]}
  Зайти в розділ купую
...
Завантажити ілюстрацію
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _add_imgs
  Клацнути по випадаючому списку   ${drop_id}
...
Додати публічний паспорт активу
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _add_assets_link
  Клацнути по випадаючому списку  ${drop_id}
  Виконати дію   ${action_id}
...
Додати офлайн документ
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _add_access_details
  Клацнути по випадаючому списку  ${drop_id}
  Виконати дію   ${action_id}
...
Додати Virtual Data Room
    ...
    Зайти в розділ списку лотів
    ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
    ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _add_vdr
    Клацнути по випадаючому списку  ${drop_id}
...
Отримати інформацію із запитання
    ...
    statusexpert.Пошук тендера у разі наявності змін   ${TENDER['LAST_MODIFICATION_DATE']}  ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Отримати інформацію із документа по індексу
  ...
  ${text}=  convert_statusexpert_string_to_common_string   ${text}
  [return]  ${text}
...
Отримати інформацію із документа
  ...
  Run Keyword And Return If  '${username}' == 'statusexpert_Owner' and '${username}' == 'description'   Fail   Опис документа відсутній на юбіз
...
Скасувати закупівлю
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${STATUSEXPERT_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}  _cancel_lot
  Клацнути по випадаючому списку  ${drop_id}
...
Отримати кількість документів в ставці
  ...
  statusexpert.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
  Зайти в розділ кваліфікація
  ${drop_id}=  Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}   _pending
...
Отримати дані із документу пропозиції
  ...
  ${doc_value}=   convert_statusexpert_string_to_common_string   ${doc_value}
  [return]  ${doc_value}
...
Перейти на форму підписання контракту
  ...
  Зайти в розділ контракти
  ${drop_id}=  Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}   _pending
  ${action_id}=   Catenate   SEPARATOR=   ${STATUSEXPERT_LOT_ID}   _publish_contract
  Wait Until Keyword Succeeds   10 x   20 s   Run Keywords
  ...   Reload Page
  ...   AND   Клацнути по випадаючому списку  ${drop_id}
  ...   AND   Element Should Be Visible   id=${action_id}
